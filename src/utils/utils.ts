...
/**
 * This is used to initialize pagination for query.
 * @param request
 * @param repo
 * @param options
 * @returns {Promise<any>}
 */
export const paginate = async (request, repo, options: any = {}) => {
    try {
        let pages = 0, total = 0;
        constants.paginationLimit = +((request.query['paginate'] || request.query['limit']) ? request.query['paginate'] || request.query['limit'] : constants.paginationLimit);
        console.log('request_url=', request.hostname, request.protocol, request.baseUrl);
        const page = +request.query.page || 1;
        const endTo = page * constants.paginationLimit;
        const query = {attributes: ['id']};
        if (options['where']) {
            query['where'] = options['where'];
        }
        console.log('paginated_where=', query);
        const record = await repo.count(query);
        console.log('record=', record);
        const from = (page === 1) ? page : (endTo > total && record === 0) ? null : endTo - constants.paginationLimit + 1;
        const to = (page === 1) ? constants.paginationLimit : (endTo > total && record === 0) ? null : endTo;
        pages = Math.ceil(record / constants.paginationLimit);
        total = record;
        const path = request.protocol + '://' + request.headers.host + request.baseUrl + '?page=';
        const next_page_url = (endTo > total) ? null : path + (page + 1);
        const prev_page_url = (page === 1 && endTo <= total) ? null : (endTo > total) ? null : path + (page - 1);
        const offset = constants.paginationLimit * (page - 1);
        console.log({count: record, pages, page, offset});
        return await {
            offset, total_pages: pages, last_page: pages, current_page: page,
            total, per_page: constants.paginationLimit, from, to, next_page_url, prev_page_url,
        };
    } catch (e) {
...
/**
 * This is used to process pagination
 * @param request
 * @param repository
 * @param options
 * @returns {Promise<{}>}
 */
export const paginatedResultSet = async (request, repository, options: object = {}) => {
    const {
        offset, total_pages, last_page, current_page,
        total, per_page, from, to, next_page_url, prev_page_url,
    } = await paginate(request, repository, options);
    console.log('info=', {offset, total_pages}, options);
    let query = {
        limit: Number(constants.paginationLimit),
        ...{offset},
    };
    ...
    // console.log('resultSet=', deepCopy(data));
    return {
        ...{
            total_pages, last_page, current_page, total,
            per_page, from, to, next_page_url, prev_page_url,
        }, ...{list: data},
    };
};
...
/**
 * This is used to convert data points options from string obj to object
 * @param datapoints
 * @returns {any}
 */
export const convertDataPointOptions = (datapoints) => {
    if (!datapoints) return datapoints;
    datapoints.forEach((datapoint, index) => {
        if (datapoint.options) {
            datapoints[index]['options'] = JSON.parse(datapoint.options);
        }
    });
    return datapoints;
};
